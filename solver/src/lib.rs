...
use clap::Parser;
use ndarray::{array, Array1, Array2, ArrayBase, Axis, Dim, ViewRepr};
...
// Impliments
impl Board {
    pub fn new(infile: InFile) -> Result<Self> {
        // get size of grid
    ...
    fn impl_requierments(
        arr: &mut Vec<CellState>,
        len: usize,
        requirements: Vec<i8>,
    ) -> Result<()> {
                ...
                match arr[i] {
                    ...
                    CellState::Unknown => {
                        if next_req.len() > 0 {
                            ...
                            total_possible.append(&mut advance(
                                &arr.clone(),
                                i,
                                next_req[0] - contig_cells,
                                len,
                                next_req[1..].to_vec(),
                                requirements.clone(),
                            ));
        ...
        if requirements.len() > 0 {
            ...
            let mut total_possible: Vec<Vec<CellState>> =
                advance(&arr, 0, 0, len, requirements.clone(), requirements.clone());
            ...
            *arr = total_possible.get(1..).unwrap_or(&vec![]).iter().fold(
                ...
                |a, b| {
                    let x = a.iter().zip(b.iter()).map(|(c, d)| *c & *d).collect();
                    // eprint!(" {:?} ",x);
                    x
                },
            );
    ...
    pub fn full_solve(&mut self, verbose: bool) -> Result<()> {
        ...
        while !self.complete() {
            if verbose {
                print!("solve # {}:", solve_num)
            };
            self.solve1(verbose).with_context(|| {
                format!(
                    "failed at solve # {}, current board:\n{}",
                    solve_num,
